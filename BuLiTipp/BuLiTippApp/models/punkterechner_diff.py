	...
	def punkte(self, tipp):
		try:
			...
			if spiel.ergebniss == tipp.ergebniss:
				...
				result = int(10-math.fabs(math.fabs(int(spiel.ergebniss.split(":")[0]) - int(tipp.ergebniss.split(":")[0]))+math.fabs(int(spiel.ergebniss.split(":")[1]) - int(tipp.ergebniss.split(":")[1]))))
				if result<1:
					result=1
				return result
